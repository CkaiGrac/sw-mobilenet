...
from mobilenet_reluhswish import Mobilenet
...
def main(args):
    ...
    with tf.variable_scope("MobileNet"):
        ...
        bn1 = tf.layers.batch_normalization(dense1, beta_initializer=tf.zeros_initializer(),
                                            ...
                                            name='bn1')
        relu = tf.nn.leaky_relu(bn1, 0.1)

        dense2 = tf.layers.dense(inputs=relu, units=10,
    ...
    loss = tf.reduce_mean(
        tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y_true, logits=sqz))
    ...
    moving_ave = tf.train.ExponentialMovingAverage(
        0.99).apply(tf.trainable_variables())

    train_filename = [os.path.join(
        './cifar', 'data_batch_%d' % i) for i in range(1, 6)]
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1, args.epochs[0]+1):
            ...
            ckpt_file = "./ckpt_reluhswish/mobileNet_test_acc=%.4f.ckpt" % test_acc
